...
RUN pip3 install --disable-pip-version-check --break-system-packages ansible==${ansible_commv} ansible-core==${ansible_version} molecule==${molecule_version} requests==${requests_version} molecule-docker docker ansible-lint flake8 yamllint pyVmomi
...
RUN echo 'user ALL=(ALL) NOPASSWD:ALL' | tee /etc/sudoers.d/20_user
USER user
